...
ip_ckpt = "interid_ckpt_path"#"
interid_model = IPAdapterFaceID_p3(pipe, ip_ckpt, device,num_tokens=4)
...
while len(face_embedding) < 3:
        face_embedding.append(torch.full_like(face_embedding[0], 0))
        for qq in range(0,4):
            face_masks.append(torch.zeros(64,64))
    

pose_result = []
inputs = tokenizer(prompt+"\npose (2ID):", return_tensors="pt")#ID num is variable lengths
inputs.to(device)
...
face_masks = face_masks.repeat(num_images_per_prompt*2,1,1,1)#8,20,64,64，negetive
cross_attention_kwargs =  {"face_masks":face_masks,}
output_images = interid_model.generate(
    ...
    scale=1,guidance_scale=3,num_samples=num_images_per_prompt, width=512, height=512, num_inference_steps=50,cross_attention_kwargs=cross_attention_kwargs)
